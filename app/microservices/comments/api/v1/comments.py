...
@wip_router.get("/{entity_id}", response_model=List[CommentResponse])
...
@wip_router.patch("/{comment_id}", response_model=UpdateCommentResponse)
...
@wip_router.post("/{comment_id}/reaction", response_model=AddReactionResponse)
async def add_reaction(comment_id: UUID, reaction: AddReactionRequest):
    db = await get_mongo_client()
    
    # Find if the reaction already exists for the comment
    existing_comment = await db["comments"].find_one({"comment_id": comment_id, "reactions.emoji": reaction.emoji})
    
    if existing_comment:
        # Increment existing reaction count
        result = await db["comments"].update_one(
            {"comment_id": comment_id, "reactions.emoji": reaction.emoji},
...
@wip_router.get("/{comment_id}/replies", response_model=GetRepliesResponse)
async def get_replies(comment_id: UUID, skip: int = 0, limit: int = 10):
    ...
    reply_ids = comment.get("replies", [])
    total_replies = len(reply_ids)  # Count total replies

    replies = await db["comments"].find({"comment_id": {"$in": reply_ids}}).skip(skip).limit(limit).to_list(length=limit)
